...
async def meta_processor(page_list, mode=None, toc_content=None, toc_page_list=None, start_index=1, opt=None, logger=None):
    ...
    if accuracy > 0.6 and len(incorrect_results) > 0:
        ...
        if mode == 'process_toc_with_page_numbers':
            ...
            # No TOC detected - create a flat structure from pages instead of failing
            logger.info('No TOC detected, creating flat page structure')
            flat_toc = []
            for i, page in enumerate(page_list, start=start_index or 1):
                flat_toc.append({
                    'title': f'Page {i}',
                    'start_index': i,
                    'end_index': i,
